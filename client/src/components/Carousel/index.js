...
import { addClass, removeClass } from "../../helpers/format/classNameMod"
...
export default function Carousel({ children, refContainer }) {
    ...
    const fnCheckIndex = useCallback(
        (e) => {
            if (e.propertyName === "left") {
                setTimeout(() => {
                    removeClass(refDragHandler.current, "trasition-all duration-200")
                }, 200);
    ...
    const fnShiftItem = useCallback(
        (direction) => {
            addClass(refDragHandler.current, "trasition-all duration-200")
    ...
    const onDragEnd = useCallback((e) => {
        ...
        if (posFinal.current - postInitial.current < -treshold) {
            fnShiftItem(DIRECTION_LEFT)
        } else if (posFinal.current - postInitial.current > treshold) {
